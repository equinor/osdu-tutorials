...
export function loadWellboreTrajectoryStartAction(
  wellboreId: string
): WellboreTrajectoryActionTypes {
  return {
...
export const loadWellboreTrajectoryAction =
  (wellboreId: string): AppThunk =>
  (dispatch) => {
    dispatch(loadWellboreTrajectoryStartAction(wellboreId));

    return loadWellboreTrajectory(wellboreId)
      .then((data) =>
        dispatch(loadWellboreTrajectorySuccessAction(wellboreId, data))
      )
      .catch((err) =>
        dispatch(loadWellboreTrajectoryFailAction(err, wellboreId))
      );
  };
